...
def train_seq(models, nblocks, optimizers, schedulers, criterion, tra, mean, taus, uza, lml0, lml0type, lml0power, uzt, uzs, ne1, ne2, 
	...
	for totrain in range(len(models)):
		trloss, vlacc = train_submodel(totrain, models, nblocks, optimizers, schedulers, criterion, tra, mean, taus, uza, lml0, lml0type, lml0power, uzt, uzs, ne1, ne2, trainloader, valloader, testloader, r)
	return trloss, vlacc
...
def blockwise_exp(modname, nfilters, traintype, enc, encodingshape, classifier, clname, apc, nclasses, nmodels, nblocks, downsample, bn, bias, smallstep, rel, res,
	...
	taus = [tau / 2] * int(nmodels / 2) + [tau] * int(nmodels / 2) if varyingtau else [tau] * nmodels
	criterion = nn.CrossEntropyLoss()
	for model in models:
		model.to(device)
...
def experiment(dataset, modelname, batchsize, nfilters, traintype, aeds, aebn, fixencoder, simpencoder, testae, clname, avgpoolcl, onecl, fixcl, nmodels, nblocks, 
	...
	if experiments and nepochs > 1:
		...
		expname = ['log', 'gflowall16', modelname, dataset, traintype, d[traintype], clname, initname, f'ing{initgain}', optimizer, f'lrt{learningrate}', 
	...
	print('experiment from gflowall16.py with parameters')
	for name in names:
...
def experiments(parameters, average):
	...
	sep = '-' * f 
	print('\n' + sep, 'gflowall16.py')
	print(sep, nexperiments, 'gflowall16 experiments ' + ('to average ' if average else '') + 'over parameters:')
	pprint.pprint(parameters, width = f, compact = True)
	for params in product([values for name, values in parameters]) :
		j += 1
		print('\n' + sep, 'gflowall16 experiment %d/%d with parameters:' % (j, nexperiments))
		...
		print(sep, 'gflowall16 experiment %d/%d over. took %.1f s. total %.1f s' % (j, nexperiments, t1, time.time() - t0))
	if average:
		acc = np.mean(accs)
...
if __name__ == '__main__':
	...
	parser.add_argument("-dat", "--dataset", required = True, choices = ['mnist', 'cifar10', 'cifar100', 'tinyimagenet', 'imagenet2012'], nargs = '*')
