...
class OneRepResNet(nn.Module):
	def __init__(self, featureshape, first, downsample, nclasses, nblocks, batchnorm, bias, smallstep, classifier, clname, apc, initialization):
		...
		self.stage = nn.ModuleList([FirstResBlock(nfilters, batchnorm, bias, h) if first and j == 0 else ResBlock(nfilters, batchnorm, bias, h) for j in range(nblocks)])
...
def get_resnet_blocks(downsample, encodingshape, nclasses, nmodels, nbl, batchnorm, bias, smallstep, classifier, clname, apc, initialization):
	...
	return [OneRepResNet(featshape(i), not i or ds(i), ds(i), nclasses, nbl, batchnorm, bias, smallstep, classifier, clname, apc, initialization) for i in range(nmodels)]
...
def get_models(modelname, initialization, traintype, downsample = 0, encodingshape = None, nclasses = 10, nmodels = 10, nblocks = 1, batchnorm = 1, bias = 0, 
	...
	if modelname == 'resnet':
		return get_resnet_blocks(downsample, encodingshape, nclasses, nmodels, nblocks, batchnorm, bias, smallstep, classifier, clname, avgpoolcl, initialization)
...
def blockwise_exp(modname, nfilters, traintype, encoder, encodingshape, classifier, clname, apc, nclasses, nmodels, nblocks, downsample, bn, bias, smallstep, rel, res,
	...
	models = [Encoder(encoder, encodingshape, nclasses, nfilters, classifier, clname, apc, bias, initialization)] 
	models += get_models(modname, initialization, traintype, downsample, encodingshape, nclasses, nmodels, nblocks, bn, bias, smallstep, classifier, clname, apc, rel, res) 
	if opt == 'adam':
...
def experiment(dataset, modelname, batchsize, nfilters, traintype, aeds, aebn, fixencoder, simpencoder, testae, clname, avgpoolcl, onecl, fixcl, nmodels, nblocks, 
	...
	if experiments and nepochs > 1:
		...
		expname = ['log', 'gflowall14', modelname, dataset, traintype, d[traintype], clname, initname, f'ing{initgain}', optimizer, f'lrt{learningrate}', 
	...
	print('experiment from gflowall14.py with parameters')
	for name in names:
...
def experiments(parameters, average):
	...
	sep = '-' * f 
	print('\n' + sep, 'gflowall14.py')
	print(sep, nexperiments, 'gflowall14 experiments ' + ('to average ' if average else '') + 'over parameters:')
	pprint.pprint(parameters, width = f, compact = True)
	for params in product([values for name, values in parameters]) :
		j += 1
		print('\n' + sep, 'gflowall14 experiment %d/%d with parameters:' % (j, nexperiments))
		...
		print(sep, 'gflowall14 experiment %d/%d over. took %.1f s. total %.1f s' % (j, nexperiments, t1, time.time() - t0))
	if average:
		acc = np.mean(accs)
